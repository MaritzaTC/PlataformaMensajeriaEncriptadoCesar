...
def enviados(correo):
    ...
    db.row_factory=sqlite3.Row
    cursor=db.cursor()
    consulta="select m.id,m.asunto,m.mensaje,m.fecha,m.hora, u.nombreusuario  from mensajeria m, usuarios u where m.origen='"+correo+"' and m.destino=u.correo"
    cursor.execute(consulta)
    return cursor.fetchall()
...
def regisUsuario(nombe,correo, password,codigo):
    try:
        ...
        db.row_factory=sqlite3.Row
        cursor=db.cursor()
        consulta="insert *from usuarios where correo='"+correo+"' and password='"+password+"' and estado='0"
        cursor.execute(consulta)
...
def deleteMensaje(id):
    ...
    db.commit()
    return "1"
def registroEMail(asunto,mensaje,origen,destino,clave):
    db=sqlite3.connect("mensajes.s3db")
    db.row_factory=sqlite3.Row
    cursor=db.cursor()
    consulta="insert into mensajeria (asunto,mensaje,fecha,hora,origen,destino,estado,clave) values ('"+asunto+"','"+mensaje+"',DATE('now'),TIME('now'),'"+origen+"','"+destino+"','0','"+clave+"')"
    cursor.execute(consulta)
    db.commit()
    return "1"
